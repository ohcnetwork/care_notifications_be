...
def notify_rescheduled(booking_id: int):
    ...
    if notif.rescheduled_sent_at is not None:
        return

    if dispatch(booking, NotificationType.reschedule):
        notif.rescheduled_sent_at = timezone.now()
        notif.save()
